<div class="flex flex-col">
    <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
        <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
            <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    @foreach($selectedFields as $keys => $selectedField)
                        @if($selectedFields[$keys]['id'])
                            @if($selectedFields[$keys]['if_level'] === 0)
                                ...
                                <tr id="if_{{$keys}}">
                                    ...
                                    <td class="hidden px-6 py-4 whitespace-nowrap">
                                        <x-jet-input class="hidden"
                                                     ...
                                                     name="selectedFields[{{ $keys }}][if_type]"
                                                     wire:model="selectedFields.{{ $keys }}.if_type" />
                                        <x-jet-input-error for="selectedFields.{{ $keys }}.if_type" class="mt-2" />
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @endif
                                        <x-jet-input-error for="selectedFields.{{ $keys }}.if_condition" class="mt-2" />

                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <x-jet-label value="{{ __($selectedFields[$keys]['if_field']) }}" />
                                        <x-jet-input-error for="selectedFields.{{ $keys }}.if_field" class="mt-2" />
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="relative inline-flex">
                                                ...
                                                <select
                                                    ...
                                                    name="selectedFields[{{ $keys }}][if_operator]"
                                                    wire:model="selectedFields.{{ $keys }}.if_operator"
                                        ...
                                        <x-jet-input-error for="selectedFields.{{ $keys }}.if_operator" class="mt-2" />
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <x-jet-label value="{{ __($selectedFields[$keys]['if_value']) }}" />
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <x-jet-button id="editField_{{ $keys }}" wire:click.prevent="$emitTo('selection-if-filters','editData', {{ $keys }}, {{ $selectedField['id'] }}, '{{ $selectedField['if_uuid'] }}')" class="bg-blue-50 hover:bg-gray-50 focus:border-white-700 focus:ring focus:ring-blue-200 active:bg-white-600" wire:loading.attr="disabled">
            ...
            <div>
                <div>
                    <x-jet-button id="addIf" name="addIf" wire:click.prevent="$emit('openIfModal')" class="ml-2" wire:loading.attr="disabled">
                        {{ __('+ Add New IF Field') }}
                    </x-jet-button>
                </div>
            </div>
